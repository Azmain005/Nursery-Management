...
const NurseryCheckout = () => {
  ...
  const updateMaterialQuantity = async (materialId, quantityToSubtract) => {
    ...
    console.log(
      `Updating material ${materialId}: current=${currentQuantity}, subtract=${quantityToSubtract}, new=${newQuantity}`
    );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      for (const item of checkoutItems) {
        if (!item.materialId) {
          console.warn(
            "Item without materialId found, skipping inventory check"
          );
          continue;
        }
        ...
        if (currentQuantity < item.quantity) {
          throw new Error(
            `Not enough stock for "${item.name}". Only ${currentQuantity} left.`
          );
        }
      }
  ...
  if (orderPlaced) {
    return (
      <div className="min-h-screen bg-[#faf6e9] flex flex-col items-center justify-center">
        ...
        <p className="mb-4">
          Thank you for your order. Your request has been submitted to the
          nursery management.
        </p>
        <NavLink
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <input
                ...
                className="input w-full border border-[#607b64]"
                required
              />
              <input
                name="lastName"
              ...
              <input
                ...
                onChange={handleChange}
                placeholder="Address*"
                className="input w-full md:col-span-2 border border-[#607b64]"
                required
              />
              <input
                name="mobile"
              ...
              <textarea
                ...
                onChange={handleChange}
                placeholder="Comment"
                className="textarea w-full md:col-span-2 border border-[#607b64]"
              />
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <label className="text-gray-600 text-sm">
              I confirm this order is correct and ready for processing.
            </label>
          </div>
